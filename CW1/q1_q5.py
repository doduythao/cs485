...
# show_arr_imgs(s1_sorted_eigenvec[:, :5].T)
# show_arr_imgs(inc_sorted_eigenvec[:, :5].T)

# show the original 1st image, batch recon, 1st subset recon, incremental recon:
show_arr_imgs([train_flat[0], train_recon[0], s1_train_recon[0], inc_train_recon[0]], titles=['Original', 'Batch', '1st subset', 'Incremental'])
# show the original last image, batch recon, 1st subset recon, incremental recon:
show_arr_imgs([train_flat[-1], train_recon[-1], s1_train_recon[-1], inc_train_recon[-1]], titles=['Original', 'Batch', '1st subset', 'Incremental'])

n_nb = 3
neigh = KNeighborsClassifier(n_nb, weights='distance', metric='euclidean')
neigh.fit(train_projs, train_y)

s1_neigh = KNeighborsClassifier(n_nb, weights='distance', metric='euclidean')
s1_neigh.fit(s1_train_projs, np.array(np.array_split(train_y, n_sub))[0])

inc_neigh = KNeighborsClassifier(n_nb, weights='distance', metric='euclidean')
inc_neigh.fit(inc_train_projs, train_y)
...
s1_test_pred = s1_neigh.predict(s1_test_projs)
s1_test_acc = accuracy_score(test_y, s1_test_pred)
print('KNN test acc: ', s1_test_acc)  # M = 100, K = 3 gave best acc.
print(precision_recall_fscore_support(test_y, s1_test_pred, average='macro'))

inc_test_pred = inc_neigh.predict(inc_test_projs)
inc_test_acc = accuracy_score(test_y, inc_test_pred)
print('KNN test acc: ', inc_test_acc)  # M = 100, K = 3 gave best acc.
print(precision_recall_fscore_support(test_y, inc_test_pred, average='macro'))

# Calculate the confusion matrix
batch_confusion_matrix = confusion_matrix(test_y, test_pred)
show_conf_mat(batch_confusion_matrix)

s1_confusion_matrix = confusion_matrix(test_y, s1_test_pred)
show_conf_mat(s1_confusion_matrix)

inc_confusion_matrix = confusion_matrix(test_y, inc_test_pred)
show_conf_mat(inc_confusion_matrix)
